(() => {
  ...
  class PianoGame {
    ...
    frame(now){if(!this.running||this.finished||this.destroyed)return;const delta=Math.min(.05,(now-this.lastFrame)/1000);this.lastFrame=now;if(!this.waiting)this.time+=delta*this.speed;this.updateNotes();const end=this.loop.enabled?this.loop.end:this.section.end;if(this.time>end){if(this.loop.enabled){this.startRun(this.loop.start,true);return;}this.finish();}else if(this.running&&!this.finished)this.raf=requestAnimationFrame(t=>this.frame(t));}
    updateNotes(){
