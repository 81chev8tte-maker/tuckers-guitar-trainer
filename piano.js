(() => {
  ...
  function renderMidiPreview(file,parsed){
    ...
    $('confirmMidiImport').onclick=async()=>{const chosen=$('midiPreview').querySelector('input:checked');if(!chosen){$('midiImportStatus').textContent='Choose a playable Piano track first.';return;}const selectedIndex=+chosen.value,selected=playable[selectedIndex],selectedAnalysis=analysis?.analyses[selectedIndex];const notes=selected.notes.filter(n=>n.channel!==9&&n.midi>=PIANO_MIN&&n.midi<=PIANO_MAX);if(!notes.length){$('midiImportStatus').textContent='That track has no valid piano notes from A0 through C8.';return;}const range=noteRange(notes),span=Math.max(1,Math.ceil((range.max-range.min+1)/12));const song={id:`midi-${uid()}`,title:file.name.replace(/\.midi?$/i,''),description:`Imported from ${file.name}`,tempo:parsed.tempo,trackName:selected.name,imported:true,notes,difficulty:selectedAnalysis?.difficulty||'Unrated',difficultyStars:selectedAnalysis?.stars||0,practiceRole:selectedAnalysis?.role||'Playable part',practiceTraits:selectedAnalysis?.traits||[]};await storeSong(song);$('midiImportStatus').textContent=`Imported ${notes.length} notes from ${selected.name}. ${selectedAnalysis?.difficulty||''} ${'⭐'.repeat(selectedAnalysis?.stars||0)} · Range: ${noteName(range.min)}–${noteName(range.max)}.${span>=5?' Song Range will follow this wide part.':''}`;preview.hidden=true;renderSongs();showPianoView('songs');};
  }
