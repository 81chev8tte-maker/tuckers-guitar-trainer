(() => {
  ...
  class PianoGame {
    ...
    updateAccompaniment(){const notes=this.song.accompaniment||[];while(this.accompanimentIndex<notes.length&&notes[this.accompanimentIndex].start<=this.time+.025){const note=notes[this.accompanimentIndex++];if(note.start>=this.section.start&&(note.start<=this.section.end)){pianoSynth.noteOn(note.midi,Math.round((note.velocity||72)*(progress.settings?.accompanimentVolume||.22)/.22),note.duration/this.speed);highlightKey($('pianoGame'),note.midi,'accompaniment',Math.max(120,note.duration*700/this.speed));}}}
    updateNotes(){
  ...
  async function startSong(id,mode='wait',sectionChoice='full',lessonId=null,handMode='full',practiceMode=null){
    ...
    try{if(currentGame)currentGame.destroy();let song=builtInSongs.find(s=>s.id===id)||await getSong(id);if(!song)return;let selected=song.notes,accompaniment=[];if(songbook&&song.notes.some(n=>n.role)){const requested=mode==='listen'?'listen':practiceMode||'melody',arranged=songbook.arrangement(song,requested,['full','left','right'].includes(requested)?'midi':'microphone');selected=arranged.player;accompaniment=arranged.accompaniment;}else{if(song.imported&&handMode!=='full')selected=selected.filter(note=>(note.hand||(note.midi<60?'left':'right'))===handMode);if(!selected.length)selected=song.notes;selected=song.imported?melodyPracticeNotes(selected):selected.map(n=>({...n}));}const clone={...song,handMode,practiceMode:practiceMode||'melody',accompaniment:accompaniment.map(n=>({...n})).sort((a,b)=>a.start-b.start||a.midi-b.midi),notes:selected.map(n=>({...n})).sort((a,b)=>a.start-b.start||a.midi-b.midi)};const rangePreference=lessonId?'beginner':song.imported?'song':(progress.settings?.rangeMode||'beginner');currentGame=new PianoGame(clone,mode,lessonId,{rangePreference});if(sectionChoice!=='full'){const start=+sectionChoice*30;currentGame.section={start,end:Math.min(start+30,Math.max(...clone.notes.map(n=>n.start+n.duration))+1)};}currentGame.mount();}finally{startSongBusy=false;}
  }
