(() => {
  ...
  class PianoGame {
    constructor(song,mode,lessonId,options={}){this.song=song;this.mode=mode;this.lessonId=lessonId;this.rangePreference=options.rangePreference||(song.imported?'song':'beginner');this.returnView=lessonId?'lessons':'songs';this.speed=1;this.index=0;this.hits=0;this.misses=0;this.combo=0;this.bestCombo=0;this.score=0;this.time=0;this.running=false;this.waiting=false;this.finished=false;this.destroyed=false;this.lastFrame=0;this.unsubscribe=null;this.raf=0;this.lookAhead=4;this.timers=new Set();this.section={start:0,end:Math.max(...song.notes.map(n=>n.start+n.duration))+1};this.displayRange={...BEGINNER_RANGE};}
    mount(){
      ...
      $('pianoExitGame').onclick=()=>this.destroy();$('pianoRestart').onclick=()=>this.restart();$('pianoPause').onclick=()=>this.togglePause();$('pianoGameSpeed').onchange=e=>this.speed=+e.target.value;$('pianoRangeMode').onchange=e=>{const actual=noteRange(this.sectionNotes());if(e.target.value==='beginner'&&(actual.min<BEGINNER_RANGE.min||actual.max>BEGINNER_RANGE.max)){e.target.value='song';this.feedback('This song needs Song Range');}this.rangePreference=e.target.value;progress.settings.rangeMode=this.rangePreference;saveProgress();this.restart();};$('pianoGameMic').onclick=()=>this.toggleMic();$('pianoPlayAgain').onclick=()=>{this.resultPanel.hidden=true;this.restart();};$('pianoResultBack').onclick=()=>{const destination=this.returnView;this.destroy();showPianoView(destination);};
    ...
    restart(){cancelAnimationFrame(this.raf);this.clearTimers();this.finished=false;this.destroyed=false;this.song.notes.forEach(n=>delete n.done);this.index=this.song.notes.findIndex(n=>n.start>=this.section.start);if(this.index<0)this.index=0;this.hits=this.misses=this.combo=this.bestCombo=this.score=0;this.time=this.section.start;this.running=true;this.waiting=false;this.lastFrame=performance.now();this.resultPanel.hidden=true;this.updateRange(true,this.song.notes[this.index]?.midi);createKeyboard($('pianoGameKeyboard'),true,this.displayRange.min,this.displayRange.max);this.makeLanes();$('pianoStage').querySelectorAll('.falling-note').forEach(n=>n.remove());$('waitCallout').hidden=true;$('pianoInputPill').textContent=`${this.displayRange.label||BEGINNER_RANGE.label} · screen keys`;this.updateHud();this.frame(this.lastFrame);}
    frame(now){if(!this.running||this.finished||this.destroyed)return;const delta=Math.min(.05,(now-this.lastFrame)/1000);this.lastFrame=now;if(!this.waiting)this.time+=delta*this.speed;this.updateNotes();if(this.time>this.section.end)this.finish();else if(this.running&&!this.finished)this.raf=requestAnimationFrame(t=>this.frame(t));}
    updateNotes(){
    ...
    togglePause(){if(this.finished||this.destroyed)return;this.running=!this.running;$('pianoPause').textContent=this.running?'Pause':'Resume';if(this.running){this.lastFrame=performance.now();this.frame(this.lastFrame);}else cancelAnimationFrame(this.raf);}
  ...
  function renderMidiPreview(file,parsed){
    const playable=parsed.tracks.filter(t=>t.notes.length);const preview=$('midiPreview');preview.hidden=false;preview.innerHTML=`<p class="eyebrow">IMPORT PREVIEW</p><h2>${escapeHtml(file.name)}</h2><p>${parsed.tempo} BPM · ${playable.reduce((n,t)=>n+t.notes.length,0)} note events</p><div class="track-picker">${playable.map((t,i)=>{const valid=t.notes.filter(n=>n.channel!==9&&n.midi>=PIANO_MIN&&n.midi<=PIANO_MAX),range=noteRange(valid);return `<label class="track-choice"><span><input type="radio" name="midiTrack" value="${i}" ${i===0?'checked':''}> <strong>${escapeHtml(t.name||`Track ${t.index+1}`)}</strong>${t.instrument?` · ${escapeHtml(t.instrument)}`:''}</span><span>${t.notes.length} notes${t.channels.includes(9)?' · drums':''}${valid.length?` · ${noteName(range.min)}–${noteName(range.max)}`:''}</span></label>`;}).join('')}</div><button id="confirmMidiImport" class="button big">Import Selected Track</button>`;
    $('confirmMidiImport').onclick=async()=>{const selected=playable[+$('midiPreview').querySelector('input:checked').value];const notes=selected.notes.filter(n=>n.channel!==9&&n.midi>=PIANO_MIN&&n.midi<=PIANO_MAX);if(!notes.length){$('midiImportStatus').textContent='That track has no valid piano notes from A0 through C8.';return;}const range=noteRange(notes),span=Math.max(1,Math.ceil((range.max-range.min+1)/12));const song={id:`midi-${uid()}`,title:file.name.replace(/\.midi?$/i,''),description:`Imported from ${file.name}`,tempo:parsed.tempo,trackName:selected.name,imported:true,notes};await storeSong(song);$('midiImportStatus').textContent=`Imported ${notes.length} notes from ${selected.name}. Range: ${noteName(range.min)}–${noteName(range.max)}.${span>=5?' Song Range will follow this wide part.':''}`;preview.hidden=true;renderSongs();showPianoView('songs');};
  }
