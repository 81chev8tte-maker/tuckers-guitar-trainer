(() => {
  ...
  songbook?.publicDomain.forEach(song=>{if(!builtInSongs.some(item=>item.id===song.id))builtInSongs.push(song);});
  ...
  async function renderSongs(){
    ...
    $('pianoSongList').innerHTML=all.map(song=>{const duration=Math.max(...song.notes.map(n=>n.start+n.duration)),range=noteRange(song.notes),octaves=Math.max(1,Math.ceil((range.max-range.min+1)/12)),difficulty=song.difficulty&&song.difficulty!=='Unrated'?` · ${'⭐'.repeat(song.difficultyStars||1)} ${song.difficulty}`:'';const sections=song.phraseBoundaries?.length?song.phraseBoundaries.map((phrase,i)=>`<option value="phrase:${i}">${escapeHtml(phrase.label)} · Measures ${phrase.startMeasure}–${phrase.endMeasure}</option>`).join(''):duration>18?Array.from({length:Math.ceil(duration/30)},(_,i)=>`<option value="time:${i*30}:${Math.min(duration,(i+1)*30)}">Section ${i+1} · ${formatTime(i*30)}–${formatTime(Math.min(duration,(i+1)*30))}</option>`).join(''):'';const arranged=Boolean(song.phraseBoundaries&&song.notes.some(n=>n.role));const actions=arranged?`<button class="button ghost" data-listen-song="${song.id}">▶ Listen First</button><button class="button" data-song="${song.id}" data-practice="melody" data-mode="wait">Learn Melody</button><button class="button secondary" data-song="${song.id}" data-practice="right" data-mode="normal">Right Hand</button><button class="button secondary" data-song="${song.id}" data-practice="left" data-mode="normal">Left Hand</button><button class="button secondary" data-song="${song.id}" data-practice="full" data-mode="normal">Hands Together</button>`:`<button class="button" data-song="${song.id}" data-practice="melody" data-mode="wait">Wait Mode · Melody</button><button class="button secondary" data-song="${song.id}" data-practice="full" data-mode="normal">Rhythm Mode · Full Part</button>`;return `<article class="piano-list-card"><div><p class="eyebrow">${song.imported?'IMPORTED MIDI':song.rights?'BUILT-IN SONGBOOK':'PRACTICE SONG'}${song.imported?` · ${song.trackName||'Selected track'}`:''}${difficulty}</p><h2>${escapeHtml(song.title)}</h2><p>${escapeHtml(song.description||`${song.notes.length} notes · ${Math.round(song.tempo||120)} BPM`)}</p>${song.rights?`<p class="piano-range-summary">${escapeHtml(song.rights.status)} · New FMQ arrangement</p>`:''}${song.practiceRole?`<p><strong>${escapeHtml(song.practiceRole)}</strong>${song.practiceTraits?.length?` · ${song.practiceTraits.map(escapeHtml).join(' · ')}`:''}</p>`:''}<p class="piano-range-summary">${song.measureCount?`${song.measureCount} measures · ${Math.round(duration)} seconds · `:''}Range: <strong>${noteName(range.min)}–${noteName(range.max)}</strong>${octaves>=5?' · Wide song: the keyboard will follow the notes.':''}${song.imported?' · Wait Mode uses a microphone-safe melody; Full Part preserves MIDI chords.':''}</p></div><div class="piano-toolbar">${sections?`<select class="piano-section-select" aria-label="Practice range"><option value="full">Full Song</option>${sections}</select>`:''}${actions}${song.imported?`<button class="button ghost" data-delete-piano-song="${song.id}">Delete</button>`:''}</div></article>`;}).join('');
  ...
  class PianoGame {
    ...
    async mount(){
      ...
      const midiButton=document.createElement('button');midiButton.id='pianoGameMidi';midiButton.className='button small secondary';midiButton.textContent='⌨ USB MIDI';root.querySelector('.piano-game-controls').appendChild(midiButton);midiButton.onclick=()=>this.connectMidi();
    ...
    updateEffectiveBpm(){const base=Math.round(this.tempoAt(this.time)),effective=Math.round(base*this.speed),label=$('pianoEffectiveBpm');if(label)label.textContent=this.speed===1?`${base} BPM`:`${base} BPM · ${Math.round(this.speed*100)}% = ${effective} BPM`;}
    frame(now){if(!this.running||this.finished||this.destroyed)return;const delta=Math.min(.05,(now-this.lastFrame)/1000);this.lastFrame=now;if(!this.waiting)this.time+=delta*this.speed;this.updateAccompaniment();this.updateNotes();const end=this.loop.enabled?this.loop.end:this.section.end;if(this.time>end){if(this.loop.enabled){this.startRun(this.loop.start,true);return;}this.mode==='listen'?this.finishDemo():this.finish();}else if(this.running&&!this.finished)this.raf=requestAnimationFrame(t=>this.frame(t));}
    updateAccompaniment(){if(progress.settings?.accompaniment===false)return;const volume=progress.settings?.accompanimentVolume??.22;if(volume<=0)return;const notes=this.song.accompaniment||[];while(this.accompanimentIndex<notes.length&&notes[this.accompanimentIndex].start<=this.time+.025){const note=notes[this.accompanimentIndex++];if(note.start>=this.section.start&&(note.start<=this.section.end)){pianoSynth.noteOn(note.midi,Math.round((note.velocity||72)*volume/.22),note.duration/this.speed);highlightKey($('pianoGame'),note.midi,'accompaniment',Math.max(120,note.duration*700/this.speed));}}}
    updateNotes(){
      ...
      const start=this.loop.enabled?this.loop.start:this.section.start,end=this.loop.enabled?this.loop.end:this.section.end;this.song.notes.forEach((note,i)=>{if(note.start<start||note.start>end)return;const until=note.start-this.time;if(until>this.lookAhead||until<-.6)return;let el=stage.querySelector(`[data-note-index="${i}"]`);if(!el){const geo=this.keyGeometry(note.midi);el=document.createElement('div');el.className=`falling-note ${note.hand||'right'}`;el.dataset.noteIndex=i;el.dataset.midi=note.midi;el.style.left=`${geo.left}%`;el.style.width=`${geo.width}%`;el.innerHTML=`<small>${note.hand==='left'?'L':'R'}</small>${noteName(note.midi)}`;stage.appendChild(el);}const noteProgress=1-until/this.lookAhead;el.style.transform=`translateY(${noteProgress*travel-54}px)`;
        ...
        const group=this.targetTracker?.current();
        if(until<=0&&this.mode==='wait'&&!note.done&&group?.items.some(item=>item.index===i)){this.waiting=true;$('waitCallout').hidden=false;$('waitCallout').querySelector('strong').textContent=[...group.required].map(noteName).join(' + ');this.requireKeys([...group.required].filter(midi=>!group.matched.has(midi)));}
      });
      const current=this.targetTracker?.current();if(current&&this.mode==='normal'&&this.time-current.start>.28)this.markMissGroup(current);
      this.syncTargetIndex();
    }
    ...
    onInput(event){if(event.type!=='noteon'||!this.running||this.finished||this.mode==='listen')return;highlightKey($('pianoGame'),event.midi);$('pianoInputPill').textContent=`Heard: ${noteName(event.midi)} · ${event.source==='microphone'?'microphone':event.source==='midi'?'USB MIDI':'screen key'}`;const group=this.targetTracker?.current();if(!group)return;const timing=group.start-this.time,validTime=this.mode==='wait'?(this.waiting||timing<=.05):Math.abs(timing)<.36;if(!validTime)return;const result=this.targetTracker.accept(event.midi);if(result.accepted){this.markGroupHit(result,timing);return;}if(result.duplicate)return;this.feedback(`Almost! Heard ${noteName(event.midi)} · Find ${[...group.required].filter(midi=>!group.matched.has(midi)).map(noteName).join(' + ')}`,'bad');highlightKey($('pianoGame'),event.midi,'wrong',250);if(this.mode==='normal')this.combo=0;this.updateHud();}
    markGroupHit(result,timing){result.items.forEach(({note,index})=>{note.done=true;const el=$('pianoStage').querySelector(`[data-note-index="${index}"]`);el?.classList.add('hit');this.schedule(()=>el?.remove(),180);});this.hits+=result.items.length;this.score+=result.items.length*(this.mode==='wait'?50:Math.max(50,100-Math.round(Math.abs(timing)*120)));if(result.complete){this.combo++;this.bestCombo=Math.max(this.bestCombo,this.combo);this.waiting=false;$('waitCallout').hidden=true;this.clearRequired();const cheers=['Great! ✓','You got it! ✓','Nice! ✓'];this.feedback(this.mode==='wait'?cheers[this.combo%cheers.length]:Math.abs(timing)<.1?'Perfect!':'Good!','good');}else{const group=this.targetTracker.current();this.requireKeys(group?[...group.required].filter(midi=>!group.matched.has(midi)):[]);this.feedback('Good — keep playing the chord','good');}this.syncTargetIndex();this.updateHud();}
    markMissGroup(group){const missed=group.items.filter(item=>!group.matched.has(item.note.midi));group.items.forEach(({note,index})=>{if(note.done)return;note.done=true;const el=$('pianoStage').querySelector(`[data-note-index="${index}"]`);el?.classList.add('missed');this.schedule(()=>el?.remove(),350);});this.misses+=missed.length;this.combo=0;this.targetTracker.missCurrent();this.feedback('Keep going — try the next one','bad');this.syncTargetIndex();this.updateHud();}
    ...
    requireKeys(midis){this.clearRequired();midis.forEach(midi=>{$('pianoGame').querySelector(`.piano-key[data-midi="${midi}"]`)?.classList.add('required');$('pianoGame').querySelector(`.piano-lane[data-midi="${midi}"]`)?.classList.add('target');});}
    ...
    destroy(){if(this.destroyed)return;this.destroyed=true;this.runToken++;this.finished=true;this.running=false;cancelAnimationFrame(this.raf);this.clearTimers();this.unsubscribe?.();this.unsubscribe=null;this.midiInput.stop();microphoneInput.stop();pianoSynth.stopAll();this.clearRequired();this.song.notes.forEach(n=>delete n.done);document.body.classList.remove('piano-game-open');$('pianoGame').hidden=true;$('pianoGame').innerHTML='';if(currentGame===this)currentGame=null;renderLessons();renderProgress();}
  }

  async function startSong(id,mode='wait',sectionChoice='full',lessonId=null,handMode='full',practiceMode=null){
    if(startSongBusy)return;startSongBusy=true;
    try{
      if(currentGame)currentGame.destroy();
      const song=builtInSongs.find(item=>item.id===id)||await getSong(id);if(!song)return;
      let selected=song.notes,accompaniment=[];
      const requested=mode==='listen'?'listen':practiceMode||handMode||'melody';
      const microphoneSafe=mode==='wait'||requested==='melody';
      if(songbook&&song.notes.some(note=>note.role)){
        const arranged=songbook.arrangement(song,requested,microphoneSafe?'microphone':'midi');selected=arranged.player;accompaniment=arranged.accompaniment;
      }else{
        if(song.imported&&handMode!=='full'&&requested!=='melody')selected=selected.filter(note=>(note.hand||(note.midi<60?'left':'right'))===handMode);
        if(!selected.length)selected=song.notes;
