...
### v2.6.16 / #43 + #44 acceptance protocol (physically passed)

The adult Dell Chromebook run passed this protocol; #43/#44 are Done per the PM record in #40. Retain the protocol for future workflow regressions. On the adult-operated Dell Chromebook, confirm the intended version, then intentionally start one clean Quick Hardware Test session.
...
## v2.6.17 / #40 — focused Piano C3/octave retest

Use the real electronic keyboard’s speaker → Dell Chromebook **internal microphone**, one key at a time. Confirm the app shows **v2.6.17**, select the actual tester’s profile, and record the keyboard sound/volume, placement, Chrome/PWA version and actual tester. Keep the normal setup comparable to the earlier failure. An adult can point out C3 (one octave below Middle C) and C4 (Middle C). The v2.6.15 adult result was 3/9 correct C3, 3/9 C4 and 3/9 no stable note; C4 was 4/4 correct. Use those as comparison evidence, not a required outcome.
...
5. Play one short **Wait for Me** run: **Lessons → Level 4 — Left Hand → Find Lower C → Start Practice**. Confirm microphone input is selected; this existing exercise contains repeated C3. Record any `Almost! Heard … · Find C3`, particularly C2/C4 or F1/other stable aliases.
