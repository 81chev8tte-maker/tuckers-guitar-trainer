...
Piano microphone detection uses a larger 4,096-sample analyser buffer and main-thread autocorrelation with confidence/stability/debounce logic. It is intentionally treated as monophonic. As of v2.6.17 (#40), fundamental selection still chooses the earliest near-equal local correlation peak and refines its lag. Its comparison allowance now uses estimated local interpolation error plus 10% of the best peak's residual mismatch, replacing v2.6.15's fixed .002 allowance. This distinguishes weak odd-harmonic evidence in clean signals while tolerating finite-noise differences between overlapping lag windows. Confidence remains the selected sampled correlation. It reuses the existing correlation buffer/scan; analyser size, cadence and input/scoring gates are unchanged. See `AUDIO_PIPELINE_REVIEW.md` for reproduction, rationale and physical limitations. The production thresholds remain conservative; candidate-pitch history is reset when a clean rounded pitch changes or the signal becomes invalid so stale readings from the previous note do not unnecessarily delay the next stable note.
...
- one persisted guided acceptance session that accumulates those three paths in any order; saving human observations completes the report without making the next path create a new session, while only the explicit confirmed new-session action resets all guided results;
...
- child-facing Piano prompts that physically anchor C4 as Middle C and locate D4–G4 beside it while structured expected-note evidence retains exact note/MIDI values;
