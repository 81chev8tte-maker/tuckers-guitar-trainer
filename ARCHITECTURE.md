...
Piano microphone detection uses a larger 4,096-sample analyser buffer and main-thread autocorrelation with confidence/stability/debounce logic. It is intentionally treated as monophonic. As of v2.6.15 (#40), fundamental selection prefers the earliest local correlation peak whose interpolated height is within .002 of the global sampled maximum, then refines that peak’s lag with three-point parabolic interpolation. Confidence remains the selected sampled correlation. A reusable correlation buffer adds one bounded linear peak scan to the existing correlation search; analyser size, cadence and input/scoring gates are unchanged. See `AUDIO_PIPELINE_REVIEW.md` for deterministic reproduction and physical limitations. The production thresholds remain conservative; candidate-pitch history is reset when a clean rounded pitch changes or the signal becomes invalid so stale readings from the previous note do not unnecessarily delay the next stable note.
