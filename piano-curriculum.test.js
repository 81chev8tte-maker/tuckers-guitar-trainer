...
curriculum.songs.forEach(song=>{
  ...
  const duration=curriculum.durationOf(song);if(song.contentType==='miniSong')assert(duration>=18,`${song.id} is too short for a mini-song`);if(song.contentType==='fullSong')assert(duration>=15,`${song.id} has an implausibly short timeline`);if(song.contentType==='checkpoint')assert(duration>=30,`${song.id} has an implausibly short checkpoint timeline`);
